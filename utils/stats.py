...
def general_stats(df):
    if df is None or df.empty:
        print("Veri seti boş veya mevcut değil.")
        return
    ...
    print(f"\nAverage Usage over 18+ Teens: {over_18['Daily_Usage_Hours'].mean():.2f} saat")
    print(f"Average Usage under 18 Teens: {under_18['Daily_Usage_Hours'].mean():.2f} saat")
    
    if 'Gender' in df.columns:
        gender_counts = df['Gender'].value_counts()
        print(f"\nGender Distrubition:")
        for gender, count in gender_counts.items():
            print(f"   {gender}: {count} kişi (%{count/total*100:.1f})")
    ...
    print(f"\nMax phone usage: {max_usage:.2f} saat")
    print(f"Min phone usage: {min_usage:.2f} saat")
